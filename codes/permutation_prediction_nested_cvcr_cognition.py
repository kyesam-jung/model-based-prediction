...
# Save results
# ------------
np.savetxt('../prediction_permutation_test/HCP_N268_' + PARAMETER_DIM + '_NestedCV_CORR_COG_' + CONDITION + '_PEARSON_R_TRAIN.txt', REPORT_TRAIN, fmt=["%.1f", "%.1f", "%.18f"])
np.savetxt('../prediction_permutation_test/HCP_N268_' + PARAMETER_DIM + '_NestedCV_CORR_COG_' + CONDITION + '_PEARSON_R_TEST.txt', REPORT_TEST, fmt=["%.1f", "%.1f", "%.18f"])
np.savetxt('../prediction_permutation_test/HCP_N268_' + PARAMETER_DIM + '_NestedCV_CORR_COG_' + CONDITION + '_CV_TRAIN_INDEX.txt', TRAIN_INDEX, fmt="%.1f")
np.savetxt('../prediction_permutation_test/HCP_N268_' + PARAMETER_DIM + '_NestedCV_CORR_COG_' + CONDITION + '_PREDICTED_Y.txt', PREDICTED_Y)
np.savetxt('../prediction_permutation_test/HCP_N268_' + PARAMETER_DIM + '_NestedCV_CORR_COG_' + CONDITION + '_BETA.txt', BETA)

if CONDITION == "EMPSIM":
    np.savetxt('../prediction_permutation_test/HCP_N268_' + PARAMETER_DIM + '_NestedCV_CORR_COG_' + CONDITION + '_SHAP_SUBJECT.txt', SHAP_SUBJECT_NP, fmt=["%.1f", "%.1f", "%.1f", "%.18f", "%.18f", "%.18f", "%.18f"])

if CONDITION == "EMP" or CONDITION == "SIM":
    np.savetxt('../prediction_permutation_test/HCP_N268_' + PARAMETER_DIM + '_NestedCV_CORR_COG_' + CONDITION + '_SHAP_SUBJECT.txt', SHAP_SUBJECT_NP, fmt=["%.1f", "%.1f", "%.1f", "%.18f", "%.18f"])

np.savetxt('../prediction_permutation_test/HCP_N268_' + PARAMETER_DIM + '_NestedCV_CORR_COG_' + CONDITION + '_SHAP_BASE.txt', SHAP_BASE_NP, fmt=["%.1f", "%.1f", "%.18f"])
